"""
Train Random Walk (Hopping) WITH LOOPING - FULL EUCLIDEAN VERSION

Same as train_random_walk.py but uses T5ModelWithLoopedHyperbolic
which loops the bottleneck layer during training. Specifically
defaulted to 'linear' for Euclidean experiments.
...
def _train_random_walk_with_looping(
    ...
    # Find longest sequence (Bu kısmı hiçbir şey silinmeden korundu)
    longest_sequence = 0
    ...
    print("Loading Model WITH FULL EUCLIDEAN LOOPING LOGIC...")
    ...
    print(f"Train with LOOPED Integrated Euclidean Model")
    ...
    config.random_walk_training.additional_log_info = f'{additional_layer}_LOOPED{num_loops}_maxnorm{max_norm}_bsize{config.t5_model.batch_size}_prompt_length{config.parse_then_hop_training.prompt_length}_lr{config.parse_then_hop_training.learning_rate}_curvature{curvature}_use_prompt_{config.random_walk_training.use_soft_prompt}'
    ...
    print(f'Random Walk Training Start...')
...
def setup_ddp(rank, world_size, master_port):
    os.environ['MASTER_ADDR'] = '127.0.0.1' # localhost yerine bu daha garanti
...
def train_ddp(rank, world_size, dataset, additional_layer, lr, curvature, knit5_checkpoint_path,
              ...
              num_loops, max_norm, input_injection, master_port): # <--- 1. BURAYA EKLE
    
    setup_ddp(rank, world_size, master_port) # <--- 2. BURAYA DA EKLE
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Random Walk Training - FULL EUCLIDEAN')
    ...
    parser.add_argument(
        ...
        default='linear', # Euclidean için Linear varsayılan yapıldı
    ...
    parser.add_argument('--no_soft_prompt', action='store_true', default=False)
    ...
    # Çoklu GPU Mp Spawn Başlatılıyor
    # Satır 228 civarı
    mp.spawn(
        train_ddp,
        args=(world_size, args.dataset, args.additional_layer, args.learning_rate, 
              ...
              args.additional_layer_lr, args.no_soft_prompt, args.use_scheduler, 
              args.checkpoint_load_path, args.tboard_logs_load_path,
              args.num_loops, args.max_norm, not args.no_input_injection, args.master_port), # <--- BURAYA EKLE
        nprocs=world_size,
        join=True
    )