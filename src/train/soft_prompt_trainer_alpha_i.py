...
class SoftPromptTrainer:
    def __init__(self,
        ...
        if self.gpu_parallelization:
            self.model.module.knit5.alphas.requires_grad = True
        else:
            self.model.knit5.alphas.requires_grad = True
        if self.gpu_parallelization:
        ...
        alphas_grad = self.model.module.knit5.alphas.requires_grad if self.gpu_parallelization else self.model.knit5.alphas.requires_grad
        print(f"Alphas learnable: {alphas_grad}")
        # optimizer_params = []
        ...
        optimizer_params.append({
            'params': [self.model.module.knit5.alphas if self.gpu_parallelization else self.model.knit5.alphas],
            'lr': config.single_hop_training.learning_rate # Using the same LR as the layer
        })
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            if self.gpu_parallelization:
                if self.rank == 0:
                    ...
                    current_alphas = self.model.module.knit5.alphas
                    print(f"Epoch {epoch} - Alpha_0 (Context): {current_alphas[0].item():.4f} | Alpha_1 (Loop): {current_alphas[1].item():.4f}")
            else:
                avg_loss = total_loss / len(self.train_dataloader)
                print(f"Epoch {epoch}, Loss: {avg_loss:.4f}")
                current_alphas = self.model.knit5.alphas
                print(f"Epoch {epoch} - Alpha_0 (Context): {current_alphas[0].item():.4f} | Alpha_1 (Loop): {current_alphas[1].item():.4f}")
        ...
        if self.gpu_parallelization:
            if self.rank == 0:
                ...
                if avg_em_perc >= self.best_em:
                    ...
                    savings = {
                        ...
                        'alphas': self.model.module.knit5.alphas,  # <--- ADD THIS LINE
            ...
            if avg_em_perc > self.best_em:
                ...
                savings = {
                    ...
                    'alphas': self.model.knit5.alphas,         # <--- ADD THIS LINE
