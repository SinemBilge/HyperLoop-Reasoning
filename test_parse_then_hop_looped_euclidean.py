"""
Evaluation script for Parse-Then-Hop with Looped Euclidean Transformer.

This script tests the full end-to-end pipeline using Euclidean looping:
...
def test_parse_then_hop_looped_euclidean(
    dataset,
    batch_size,
    ...
    """
    Test the full parse-then-hop pipeline using looped Euclidean T5 models.
    """
    ...
    # Initialize models with Euclidean looping
    print("Loading Parsing Model WITH EUCLIDEAN LOOPING...")
    print(f"  Layer type: linear (Euclidean)")
    print(f"  Num loops: {num_loops}, Input injection: {input_injection}")

    parsing_knit5_model = T5ModelWithLoopedHyperbolic(
        layer_type='linear',  # Euclidean layer
    ...
    print("Loading Hopping Model WITH EUCLIDEAN LOOPING...")
    print(f"  Layer type: linear (Euclidean)")
    print(f"  Num loops: {num_loops}, Input injection: {input_injection}")
    ...
    # Run evaluation
    print("\n" + "="*60)
    print("Starting Parse-Then-Hop Evaluation WITH EUCLIDEAN LOOPING")
    print("="*60)
    ...
    df.to_csv('pred_vs_label_path_euclidean.csv', sep=';')
    print(f"\nPredictions saved to pred_vs_label_path_euclidean.csv")

    return pred_vs_label


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Parse-Then-Hop Evaluation with Euclidean Looping')
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Checkpoint path for parsing soft prompt (Euclidean)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Checkpoint path for hopping soft prompt (Euclidean)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='Number of times to loop the Euclidean layer (default: 4)'
    )
    ...
    test_parse_then_hop_looped_euclidean(
